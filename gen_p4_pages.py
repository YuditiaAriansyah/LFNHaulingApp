...
PAGES = {
    "master-tax.html": {
        ...
        "icon": "bi-receipt",
        "title": "Tax Master",
        "subtitle": "Kelola tarif pajak (PPN, PPh) dan kode akun pajak",
        "api": "/api/taxes/master",
        "fields": [
            {"name": "TaxCode", "label": "Kode Pajak", "col": 4, "type": "text", "required": True},
            {"name": "TaxName", "label": "Nama Pajak", "col": 8, "type": "text", "required": True},
            {"name": "TaxType", "label": "Tipe Pajak", "col": 4, "type": "select", "options": ["VAT", "INCOME_TAX", "SALES_TAX", "NONE"]},
            {"name": "TaxRate", "label": "Tarif (%)", "col": 4, "type": "number"},
            {"name": "TaxRateType", "label": "Jenis Tarif", "col": 4, "type": "select", "options": ["PERCENTAGE", "FIXED"]},
            {"name": "TaxBase", "label": "Dasar Pajak", "col": 6, "type": "select", "options": ["EXCLUSIVE", "INCLUSIVE"]},
            {"name": "ApplicableTo", "label": "Berlaku Untuk", "col": 6, "type": "select", "options": ["ALL", "GOODS", "SERVICES"]},
            {"name": "CoaCode", "label": "Kode Akun (COA)", "col": 6, "type": "text"},
            {"name": "Remarks", "label": "Remarks", "col": 12, "type": "textarea"},
        ],
        "columns": [
            {"header": "Kode", "data": "taxCode"},
            {"header": "Nama Pajak", "data": "taxName"},
            {"header": "Tipe", "data": "taxType"},
            {"header": "Tarif (%)", "data": "taxRate"},
            {"header": "Jenis", "data": "taxRateType"},
            {"header": "Basis", "data": "taxBase"},
            {"header": "Akun COA", "data": "coaCode"},
            {"header": "Status", "data": "status", "badge": True},
        ],
        "stats": [
            {"id": "statTotal", "label": "Total Tax"},
            {"id": "statActive", "label": "Aktif"},
            {"id": "statVAT", "label": "PPN"},
            {"id": "statPPh", "label": "PPh"},
        ],
        "extra_stat": "document.getElementById('statVAT').textContent = data.filter(d => d.taxType === 'VAT').length;\n    document.getElementById('statPPh').textContent = data.filter(d => d.taxType === 'INCOME_TAX').length;",
    },
    "master-coa.html": {
        "accent": "#7c3aed",
        "icon": "bi-book-half",
        "title": "Chart of Accounts (COA) Master",
        "subtitle": "Kelola kode akun GL untuk jurnal transaksi",
        "api": "/api/coa/master",
        "fields": [
            {"name": "AccountCode", "label": "Kode Akun", "col": 4, "type": "text", "required": True},
            {"name": "AccountName", "label": "Nama Akun", "col": 8, "type": "text", "required": True},
            {"name": "AccountType", "label": "Tipe Akun", "col": 4, "type": "select", "options": ["ASSET", "LIABILITY", "EQUITY", "REVENUE", "EXPENSE"]},
            {"name": "AccountCategory", "label": "Kategori", "col": 4, "type": "select", "options": ["OPERATIONAL", "NON_OPERATIONAL"]},
            {"name": "AccountLevel", "label": "Level Akun", "col": 4, "type": "number"},
            {"name": "ParentAccountCode", "label": "Kode Induk", "col": 6, "type": "text"},
            {"name": "NormalBalance", "label": "Saldo Normal", "col": 3, "type": "select", "options": ["DEBIT", "CREDIT"]},
            {"name": "CostCenterRequired", "label": "Butuh Cost Center?", "col": 3, "type": "select", "options": ["N", "Y"]},
            {"name": "SiteCode", "label": "Site Code", "col": 3, "type": "text"},
            {"name": "Currency", "label": "Mata Uang", "col": 3, "type": "text"},
            {"name": "TaxCode", "label": "Kode Pajak", "col": 6, "type": "text"},
            {"name": "Description", "label": "Deskripsi", "col": 12, "type": "textarea"},
        ],
        "columns": [
            {"header": "Kode Akun", "data": "accountCode"},
            {"header": "Nama Akun", "data": "accountName"},
            {"header": "Tipe", "data": "accountType"},
            {"header": "Kategori", "data": "accountCategory"},
            {"header": "Level", "data": "accountLevel"},
            {"header": "Induk", "data": "parentAccountCode"},
            {"header": "Saldo", "data": "normalBalance"},
            {"header": "CC?", "data": "costCenterRequired"},
            {"header": "Status", "data": "status", "badge": True},
        ],
        "stats": [
            {"id": "statTotal", "label": "Total Akun"},
            {"id": "statActive", "label": "Aktif"},
            {"id": "statAsset", "label": "Aset"},
            {"id": "statExpense", "label": "Beban"},
        ],
        "extra_stat": "document.getElementById('statAsset').textContent = data.filter(d => d.accountType === 'ASSET').length;\n    document.getElementById('statExpense').textContent = data.filter(d => d.accountType === 'EXPENSE').length;",
    },
}
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto flex-wrap">
                    ...
                    <li class="nav-group">
                        <span class="nav-group-label">P4 Master</span>
                        <a class="nav-link" href="master-tax.html"><i class="bi bi-receipt"></i> Tax</a>
                        <a class="nav-link" href="master-coa.html"><i class="bi bi-book-half"></i> COA</a>
